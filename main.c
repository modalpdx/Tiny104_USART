 ...
 * BUILDING BLOCKS:
 *
 * Written for an Atmel Xplained Nano ATTiny104 eval board. This board includes 
 * a TPI programmer (no debugger), USB connectivity, one button, and one LED. It
 * also has 1k of flash and 32 bytes of RAM, which is tight. All communication
 * happens through the single USART built into the ATTiny104 MCU. The pins
 * for this USART are also routed through the USB connector so connecting via a
 * micro USB cable works fine (no jumper wires or USB/TTL devices are required).
 *
 * If you are not using that board, you will need to adjust ports, pins, etc.
 *
 *
 ...
 * to program this MCU. On Windows, I highly recommend you use
 * RealTerm as your terminal program. It offers significantly more granular
 * functionality than basic serial terminals. Programmers will love it.
 *
 * RealTerm's "Send" tab can send byte strings (enter the hex or 8bit numeric
 * value in the form field and click "Send Numbers"). If you're not on Windows,
 * find a terminal program that allows sending bytes. SENDING A BYTE WITH SET
 * BITS IS THE ONLY WAY TO TURN ANYTHING ON/OFF.
 *
 * Component bits currently look like this:
 *
 *    LED 0   (built-in, on PA5) is bit 0
 *    LED 1   (external, on PA7) is bit 1
 *    RELAY 1 (external, on PA6) is bit 2
 * 
 ...
 *
 * I suspect these are no longer available, but reed relays are pretty common and
 * usually cheap so don't be scared to use a replacement. Just be sure it takes 5v
 * on the coil part, which is what the MCU will interact with. This particular relay
 * can switch power up to 125VAC which is creepy, but it worked just fine with a 
 * boring LED which is more my speed.
 *
 * Setting a bit for a component and then sending the resulting byte to the board
 * will turn on that component only. Unset the bit and send the resulting byte to 
 * turn it off.
 *
 * NOTE: The on/off status of ALL COMPONENTS must be sent in each byte. State is 
 * not saved between received bytes! Whatever is in the byte that is received
 * will determine what is on and what is off.
 *
...
#define MYUBRR (((( FOSC / 16) + ( BAUD / 2) ) / ( BAUD )) - 1)// Component bit positions in user byte
#define C_LED0   0 
#define C_LED1   1
#define C_RELAY1 2 
...
int main(void)
{
	...
	//
	// Configure LED0, LED1, and RELAY1 pins as output (PA5, PA6, & PA7)
...
ISR(USART_RXC_vect, ISR_BLOCK)
{
	...
	// Turn RELAY 1 on/off
	if(cmd & 1<<C_RELAY1) 
		PORTA |= RELAY1;    // Turn on RELAY 1
	else 
		PORTA &= ~(RELAY1); // Turn off RELAY 1
}
